...
//#endregion
//#region electron/services/git-service.ts
var GitService = class {
	constructor(root) {
		this.git = simpleGit(root || "E:/project/obsidian-wiki");
	}
	async status() {
	...
	async pull() {
		...
		return result.summary?.changes?.length ? `拉取 ${result.summary.changes.length} 个变更` : "已是最新";
	}
	async push() {
		await this.git.push();
		return "推送成功";
